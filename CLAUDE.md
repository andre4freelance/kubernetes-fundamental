...
- This repo is **not tied to any one cluster**: the manifests are generic and get applied to whichever practice cluster is currently in use (RKE2/Rancher, kubeadm cross-cloud lab, or others). Multiple practice clusters exist and the one in use **changes session to session by the user's choice** — never assume the previous session's cluster still applies. Each reached through its own **isolated kubeconfig** kept **out of git**. Kubeconfig paths and cluster endpoints are machine-local — see `knowledge/runbooks/kubectl-context-safety.md` in `ai-ops`, not this file.
- **Ask which cluster/context this session uses before any command** — do not infer from a previous session, from `LEARNING.md`, or from whatever context the `kubernetes` MCP server happens to currently be pinned to. Once known, sync it into the MCP config for all AI tools (`ai-ops/.mcp.json`, `ai-ops/opencode.json`: `KUBECONFIG_PATH`/`K8S_CONTEXT`) and have the user restart/reconnect the MCP server. **Always run `kubectl config current-context` before any mutating command** (`apply`/`delete`/`scale`/…) and confirm it matches exactly. Never point learning applies at a production context — the workstation's *default* kubeconfig is a client production EKS cluster.
